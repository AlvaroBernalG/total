...
describe('tp', () => {
	test('--tp. Character count.', () => {
		return new Promise((resolve: any, reject: any) => {
			exec('echo "hello\nworld" | node ./dist/total.js --tp ', (err: any, stdout: string, stderr: any) => {
	...
	test('--tp L. Longest line.', () => {
		return new Promise((resolve: any, reject: any) => {
			exec('echo "hello\nworld" | node ./dist/total.js --tp L', (err: any, stdout: string, stderr: any) => {
				if (err) return reject(err);
				if (stderr) return reject(stderr);
	...
	test('--tp w. Word count.', () => {
		return new Promise((resolve: any, reject: any) => {
			exec('echo "hello\nworld th3d" | node ./dist/total.js --tp w', (err: any, stdout: string, stderr: any) => {
				if (err) return reject(err);
				if (stderr) return reject(stderr);
